...
def ssim(img1, img2, window_size=11, size_average=True, inst_mask=None):
    channel = img1.size(-3)
    ...
    return _ssim(img1, img2, window, window_size, channel, size_average, inst_mask)
...
def _ssim(img1, img2, window, window_size, channel, size_average=True, mask=None):
    ...
    if mask is not None:
        mask = F.max_pool2d(mask, kernel_size=window_size, stride=1, padding=window_size // 2)
        mask = 1.0 - mask
