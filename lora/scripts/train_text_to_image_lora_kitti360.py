...
def parse_args():
    ...
    # Sanity checks
    if dataset_name is None and args.train_data_dir is None:
...
def main(output_dir, dataset_name):
    args = parse_args()
    ...
    lr_scheduler = get_scheduler(
        ...
        num_training_steps=args.max_train_steps * args.gradient_accumulation_steps * 10,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                sample_save_dir = os.path.join(output_dir, "sample")
                os.makedirs(sample_save_dir, exist_ok=True)
                images = (torch.from_numpy(np.stack([np.asarray(img) for img in images]))/255).permute(0, 3, 1, 2)
                torchvision.utils.save_image(images, sample_save_dir + f"/sample_step_{global_step}.png")
    ...
    generator = torch.Generator(device=accelerator.device).manual_seed(args.seed)
    images = []
    for _ in range(args.num_validation_images * 2):
...
if __name__ == "__main__":
    ...
    with open(seq_metadata_path) as f:
        for line in f:
            seq = line.split("/")[-3]
            frames = line.split(".")[0].split("/")[-1]
            start_frame, end_frame = frames.split("_")
            start_frame, end_frame = str(int(start_frame)).zfill(10), str(int(end_frame)).zfill(10)
